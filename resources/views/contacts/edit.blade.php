        ...
        <div class="bg-white shadow-lg rounded p-6">
            <form action="{{ route('contacts.update',$contact) }}" method="post" autocomplete="off">
                ...
                <div class="mb-4">
                    ...
                    <input type="email" name="email" value="{{ old('email',$contact->user->email) }}"
                        placeholder="ingrese el correo electronico"
                        class="block w-full px-4 py-2 mt-2 text-gray-700 bg-white border border-gray-200 rounded-md dark:bg-gray-800 dark:text-gray-300 dark:border-gray-600 focus:border-blue-400 focus:ring-blue-300 focus:ring-opacity-40 dark:focus:border-blue-300 focus:outline-none focus:ring">
                </div>
                <div class="flex justify-end">
                    <button class="px-6 py-2 leading-5 text-white transition-colors duration-200 transform bg-emerald-700 rounded-md hover:bg-emerald-800 focus:outline-none focus:bg-emerald-800 uppercase text-sm font-semibold">
                        Actualizar contacto
                    </button>
                </div>
            </form>
